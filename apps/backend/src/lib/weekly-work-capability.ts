  ...
  const result = await pool.query<CapabilityProbeRow>(
    `
      SELECT
        ...
        to_regclass('public.idx_project_weekly_work_items_project_date') IS NOT NULL AS project_date_index,
        COALESCE((
        ...
        EXISTS (
          ...
          WHERE constraint_row.conrelid = to_regclass('public.project_weekly_work_items')
            AND constraint_row.contype = 'c'
            AND pg_get_constraintdef(constraint_row.oid) ILIKE '%version%> 0%'
        ...
        EXISTS (
          ...
          WHERE constraint_row.conrelid = to_regclass('public.project_weekly_work_items')
            AND constraint_row.contype = 'c'
            AND pg_get_constraintdef(constraint_row.oid) ILIKE '%category%leveling%manual%other%'
        ) AS category_check,
        EXISTS (
          SELECT 1
          FROM pg_constraint constraint_row
          WHERE constraint_row.conrelid = to_regclass('public.project_weekly_work_items')
            AND constraint_row.contype = 'c'
            AND pg_get_constraintdef(constraint_row.oid) ILIKE '%status%planned%in_progress%done%blocked%'
        ) AS status_check
    `
  );
