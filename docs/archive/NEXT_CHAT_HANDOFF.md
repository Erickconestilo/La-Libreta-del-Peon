...
- Verificación real cerrada en Galaxy:
  ...
  - flujo `Memoria visual -> Cámara -> disparar -> Aceptar` repetido en `***REMOVED*** Estacionamiento CN2`;
...
- QA real en Galaxy con sesión `admin` activa:
  - Arranque en frío: `Obras` cargó ***REMOVED*** y ***REMOVED*** sin quedarse en `Cargando` ni pedir `Revalidar token`.
  - Foto de obra: cambio de imagen con JPG válido desde galería pasó; se restauró ***REMOVED*** usando historial de `project.image_url`.
  - Foto principal de estación `***REMOVED*** Estacionamiento CN2`: añadir desde galería pasó; luego se quitó y volvió a `Sin foto`.
