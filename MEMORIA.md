...
| **2 — Base fiable y offline** | Motor que usan todas las operaciones | **Motor offline cerrado (28-07-2026).** SQLite versionado, outbox persistente, sync engine (flush/retry/backoff/conectividad), estados pending/syncing/synced/error/conflict, mock de test respaldado por SQLite real (`node:sqlite`) — 25/25 tests pasan, incluidos los 2 bugs reales que ese mock expuso y ya se corrigieron (backoff en UTC, desempate de orden). `client_request_id` en backend **implementado, migrado y verificado** (23/23 tests, `tsc` limpio, migración 018 aplicada a Supabase el 29-07-2026 con autorización explícita de Erick — columna + índice único parcial confirmados, 3 filas existentes intactas). No hay test de integración real contra Postgres para el `ON CONFLICT` en sí (solo revisión de código + estructura de datos verificada). **Fase 2 completa sigue abierta:** falta la prueba end-to-end en dispositivo real por cable (pendiente de Codex) — ese paso valida el `ON CONFLICT` con datos reales y cierra Fase 2 del todo. |
