...
- `Obras` carga datos reales tras instalar; visible `***REMOVED***` y `***REMOVED***`.
...
- Build móvil lanzada:
  ...
  - Estado: `FINISHED`.
  - Instalación ADB en Galaxy `SM_S938B / R5CY21X6FLE`: `Success`.
  - Mini-QA tras instalar: app arranca, proceso activo, `Obras` carga sin quedarse en `Cargando obras...`, visibles `***REMOVED***` y `***REMOVED***`.
...
- Instalación Galaxy:
  ...
  - `logcat` filtrado: sin `AndroidRuntime`, sin errores JS; solo `ReactNativeJS: Running "main"`.
  - Mini-QA: `Obras` carga con `***REMOVED***`, `***REMOVED***`, `Nueva obra` y `Añadir imagen`; sesión admin persistida.
...
- QA topógrafo:
  ...
  - Mensaje creado y confirmado en `***REMOVED*** Estacionamiento CN2`: `e1037d58-f6e6-42c2-a222-c8e8fc389003`.
...
- Entrar en `***REMOVED***`, `***REMOVED***` o `***REMOVED***`.
...
- Obra `***REMOVED***` abre con 4 estacionamientos.
- Detalle `Estacionamiento Norte ***REMOVED*** E02` abre correctamente.
...
- `Obras -> ***REMOVED***` carga la obra y sus 2 estacionamientos.
- `***REMOVED*** Estacionamiento CN2` abre correctamente.
...
- `Obras` tras arranque en frío: cargó ***REMOVED*** y ***REMOVED***; no se reprodujo cuelgue infinito en `Cargando obras`.
...
- Fotos validadas con JPG QA válido desde galería:
  - Imagen de obra: subida y PATCH pasaron. ***REMOVED*** se restauró después con historial de `project.image_url`.
...
- Obras con sesión técnica:
  - `***REMOVED***` visible y navegable;
  - acciones de edición visibles para rol técnico.
- Edición técnica validada:
  - foto principal de estación `***REMOVED*** Estacionamiento CN2` cambiada desde galería con éxito;
...
- Fix validado en Galaxy sobre `***REMOVED*** Estacionamiento CN2`.
- Flujo repetido de extremo a extremo:
  ...
  - pulsar `Aceptar`.
- Resultado observado:
  - la app vuelve al detalle correcto de `***REMOVED*** Estacionamiento CN2`;
