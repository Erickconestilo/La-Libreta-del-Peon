...
| **2 — Base fiable y offline** | Motor que usan todas las operaciones | **Motor offline cerrado (28-07-2026).** SQLite versionado, outbox persistente, sync engine (flush/retry/backoff/conectividad), estados pending/syncing/synced/error/conflict, mock de test respaldado por SQLite real (`node:sqlite`) — 25/25 tests pasan, incluidos los 2 bugs reales que ese mock expuso y ya se corrigieron (backoff en UTC, desempate de orden). `client_request_id` en backend **implementado y verificado** (23/23 tests, `tsc` limpio — Cowork, ver §9 28-07-2026), pero **la migración 018 NO está aplicada a Supabase todavía** (requiere autorización explícita, regla 6) y no hay test de integración real contra Postgres para el `ON CONFLICT`. **Fase 2 completa sigue abierta:** falta (a) aplicar la migración 018 a Supabase cuando Erick lo autorice, y (b) prueba end-to-end en dispositivo real por cable (pendiente de Codex) — ese mismo paso serviría para validar el `ON CONFLICT` con datos reales. |
