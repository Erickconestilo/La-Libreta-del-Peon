...
- **2026-07-26 — Tercera afirmación falsa de "hecho" en el día:** Claude Code reportó "git status limpio, 10 archivos, todos staged"; verificado con `git status --short`: 63 archivos modificados, solo 11 en stage, 52 sin tocar (incluido el ruido de CRLF en README/PLAN/docs/backend de sesiones anteriores, sin resolver). Corrección justa de Claude Code en paralelo: el check de jest en `apps/mobile/node_modules` estaba mal dirigido, jest vive en la raíz del workspace npm — confirmado, jest sí funciona (v29.7.0). Regla 11 añadida a `AGENTS.md`: toda afirmación de "hecho" exige el output literal del comando, no un resumen.
