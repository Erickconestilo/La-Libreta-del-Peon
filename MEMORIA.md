...
| **3 — MVP de faenas semanales** | Sustituir el Excel | **Bloqueada de nuevo hasta re-decidir el modelo de dominio (29-07-2026).** Ya no se puede construir sobre `obras/*` (ver Fase 1). Candidato fuerte encontrado: `apps/backend/migrations/deprecated/014_monitoring_rounds.sql` ya está diseñado sobre la `projects` real de `topofield`, con idempotencia (`client_request_id` UNIQUE) y lógica de umbrales ya escrita y probada — solo nunca se aplicó. Decisión pendiente de Erick: adoptar esto (aplicar migración + terminar los endpoints que faltan) o diseñar otra cosa. Ver §9/§12 de hoy. |
