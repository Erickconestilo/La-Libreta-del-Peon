...
test('seeds only neutral sample catalog, control points, and thresholds', async () => {
  ...
  assert.equal(calls.filter((call) => call.sql.includes('project_code_catalog')).length, 5);
  ...
  assert.deepEqual(calls[4].params, ['project-1', 'EJ-C-001', 'Zona Centro', 'green', 3, 1, 'surface', '0+000']);
  assert.deepEqual(calls[7].params, ['project-1', 'EJ-PIE-01', 'Ejemplo - Punto piezometrico 01', 'surface', '0+050', 'Itinerario de ejemplo 2', 'Zona Sur', 'Datos de muestra. Revise y sustituya estos valores antes de una campana real.']);
  assert.deepEqual(calls[8].params, ['point-2', 'piezometer', 10, 20, 'kPa', 'user-1']);
});
